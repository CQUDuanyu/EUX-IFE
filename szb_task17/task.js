...
function renderChart() {
  ...
  for(var i in chartData){
      ...
      str +="<div class='histogram' style='height:"+chartData[e]+"px;background-color:"+getRandomColor()+"' title ='"+ e +":"+chartData[e]+"'></div>";
  }
  document.getElementsByClassname("aqi-chart-warp")[0].innerHTML = str;
}
...
function graTimeChange() {
  // 确定是否选项发生了变化 
  var type = document.getElementsByName("gra-time").value;
  var timeNow = [];
  timeNow.forEach(function(e){
      if(e.checked){
        timeNow=e.value;
      }
  })
...
function citySelectChange() {
  // 确定是否选项发生了变化 
  var city  = document.getElementsById("city-select").value;
      ...
      case "month" :
          ...
          for (var i in aqiSourceData[city]){
            ...
            if(month==-1){
              ...
              chartData[month+"月"] = Math.round(total/round);
              month = date.getMonth()+1;
              count=0;
              total=0;
            }
...
function randomBuildColor(){
  return '#' +(function(n){
    return new Array(7-n.length).jion("0")+n
